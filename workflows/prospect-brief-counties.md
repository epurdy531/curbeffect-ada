# Workflow #2d — Prospect Brief (County Governments)

## Purpose
Find **5 new ADA Title II county governments**, look up a contact for each,
add them to the tracker, and save ONE summary draft for Erica that lists all
5 prospects with the scan command for each. Erica runs the scan
command per prospect; the scan task itself composes and emails the per-entity
outreach draft (with veraPDF stats filled in from the worst-failing PDF).
Erica reviews each composed outreach draft and sends it.
...
### 1. Find 5 new county governments in IL / IA / MN
Use web search to identify 5 distinct **county governments** (county
boards / boards of supervisors / boards of commissioners — the county
itself, not a city or special district within it) located in **Illinois,
Iowa, or Minnesota only**. Favor counties likely to have many publicly
posted PDFs in a consistent format (board agendas, minutes, resolutions,
ordinances, budgets, public notices), since that fits the free-remediation
...
Section 504 coordinator, county administrator / manager, IT / technology
director, communications director, or county clerk, with a direct email and
job title. If no named individual is publicly listed, use the most relevant
department or role-based contact and note that it is role-based.
...
tmp/new-prospects-counties-YYYY-MM-DD.md (create the file; one row per entity,
...
Use exactly this structure:
    ...
    Subject: prospect brief (counties) for YYYY-MM-DD
...
drafts/prospect-brief-counties-YYYY-MM-DD.md using today's date. Do NOT
...
## Notes
- Every entity must genuinely be subject to ADA Title II. County governments
  qualify (state/local government). No private entities or federal agencies.
...
- County-level entities only — the county government itself, not cities,
  school districts, or special districts within the county.
- **Scope: Illinois, Iowa, or Minnesota only.** Out-of-scope states are
  marked "ON HOLD (out-of-scope state, ...)" in contact-log.md; do not
  re-surface them.
- If you can't find 5 eligible counties in IL/IA/MN that aren't already
  in the tracker, include as many as you can and note "(found N of 5)" at
  the top of the list. Do NOT cross into other states to fill the count.

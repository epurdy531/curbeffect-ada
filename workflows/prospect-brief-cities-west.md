# Workflow #2b — Prospect Brief (Cities, batch B)

## Purpose
Find **5 more new ADA Title II city governments in Illinois, Iowa, or
Minnesota** (different from whatever the earlier `cities-east` batch added
today), look up a contact for each, add them to the tracker, and save ONE
summary draft for Erica that lists all 5 prospects with the scan
command for each. Erica runs the scan command per prospect; the scan task
itself composes and emails the per-entity outreach draft (with veraPDF
stats filled in from the worst-failing PDF). Erica reviews each composed
outreach draft and sends it.
...
> Note on naming: the slug `cities-west` is retained from when the cities
> work was split east/west of the Mississippi. As of 2026-06-15 both cities
> briefs target the same IL/IA/MN scope; this is "cities batch B" — the
> later-running slot. It picks 5 cities that the earlier "cities batch A"
> (slug `cities-east`) didn't already add to the outreach-db.
...
earlier today by the cities-east brief or any other category brief).
Entries tagged [DO NOT CONTACT] are parked and must not be re-surfaced.
...
tmp/new-prospects-cities-west-YYYY-MM-DD.md (create the file; one row per entity,
...
Use exactly this structure:
    ...
    Subject: prospect brief (cities batch B) for YYYY-MM-DD
    ...
    the scan command below: it scans up to 5 of the entity's PDFs through
    veraPDF, then composes an outreach draft (using
...
drafts/prospect-brief-cities-west-YYYY-MM-DD.md using today's date (the
`cities-west` slug is kept so the existing runner + plist continue to pick
...
- If you can't find 5 eligible cities in IL/IA/MN that aren't already in
  the tracker (after the earlier cities-east batch ran), include as many
  as you can and note "(found N of 5)" at the top of the list. Do NOT
  cross into other states to fill the count.
